...
function AnimationBackend(options, backend) {
    ...
    this._ticker = new PTic(this._interval).on('tick', this._tick.bind(this));
}
...
AnimationBackend.prototype._schedule = function (transition) {
    ...
    if (timeOffset) {
        ...
        process.nextTick(invocationCandidate);
    }
};
...
AnimationBackend.prototype._tick = function () {
    ...
    var nextState = transition.to().values;

    Object.keys(nextState).forEach(function (stateKey) {
        var value = nextState[stateKey];
        that._state[stateKey] = parseInt(initState[stateKey] * (1 - perc) + value * perc, 10);
    });
...
AnimationBackend.prototype._update = function () {
    if (this._backend && this._dirty) {
        ...
        this._debug('writing ', this._state);
        this._backend.update(this._state, function () {
            //do something?
        });
    }
};
